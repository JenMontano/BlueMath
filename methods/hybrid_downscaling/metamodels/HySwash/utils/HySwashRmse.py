            ...
            if var == "Ru2":
                ...
                pcs_test_predicted_ds = xr.Dataset(
                    ...
                    coords={
                        ...
                        "n_component": (["n_component"], range(1,len(pcs_test_predicted.columns)+1)),
                    })
                
                reconstructed_test_data = pca[var].inverse_transform(PCs=pcs_test_predicted_ds)
                rmse_values = []
                for i in reconstructed_test_data.case_num.values:
                    rmse_var=rmse(reconstructed_test_data.sel(case_num=i)[var].values,test_data.sel(case_num=i)[var].values)
